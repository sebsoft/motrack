...
class DirectoryHandler(SimpleHTTPRequestHandler):
    ...
    def list_directory(self, path):
        ...
        for entry in list:  # See if there is a file for initializing iframe
            ...
            if os.path.islink(fullname) or os.path.isfile(fullname) and imagelist in fullname:
                file_found = True
                break
            cnt += 1
        ...
        f.write(b'<link href="/speedpi.css" rel="stylesheet">')
        f.write(b'<script type="text/javascript" src="/script.js"></script>')
        f.write(b'</head>')
        ...
        if file_found:  # file was found display it in left pane
            f.write(b'<div id="container" name="container" ><img src="%s" id="img" alt="%s" /></div>' %  (list[cnt].encode('utf-8'), web_page_title.encode('utf-8')))
        else:  # No files found so blank left pane
            f.write(b'<div id="container" name="container" ><img src="%s" id="img" alt="%s" /></div>' %  (b"/logo.svg", web_page_title.encode('utf-8')))

        
        #refresh button
        refresh_button = ('''<FORM id="refreshbutton">&nbsp;&nbsp;<INPUT TYPE="button" onClick="history.go(0)" VALUE="Refresh">&nbsp;&nbsp;<b>%s</b></FORM>''' % list_title)
        ...
        f.write(b'<ul name="menu" id="menu">')
        ...
        for name in list:
            ...
            # handle the display of files
            else:
                if  imagelist in name:
                    f.write(b'<li ><a onclick="changeScr(\'%s\');" href="#" > %s   </a> </li>\n'
                            % (urllib.parse.quote(linkname).encode('utf-8'),  html.escape(displayname).encode('utf-8') ))
        ...
        if web_page_refresh_on:
            f.write(b'<div style="float: left; ">Auto Refresh = %s sec</div>' % web_page_refresh_sec.encode('utf-8'))

        if web_max_list_entries > 1:
            f.write(b'<div style="text-align: right; ">Listing Only %i of %i Files in %s</div>'
                    % (display_entries.encode('utf-8'), all_entries.encode('utf-8'), self.path.encode('utf-8')))
        else:
            f.write(b'<div style="text-align: right; ">Listing All %i Files in %s</div>'
